...
<context>
  Estamos creando una aplicación de consola en.NET 10 basada en Clean Architecture. 
  Ya existen las capas de Core (UseCases) y Application (Implementaciones).
  El negocio requiere comenzar a identificar quiénes realizan las compras para futuras estrategias de lealtad. Se espera que el usuario pueda diseñar nuevas funciones futuras pero al ser un trabajo colaborativo aun no existe las funciones clave diseñadas.
  
  Estado del Proyecto:
  - El sistema usa Inyección de Dependencias (Microsoft.Extensions.DependencyInjection).
  - Los casos de uso de Productos (Consultar todos, Consultar por ID, Registrar) ya serán implementados PERO ESE NO ES NUESTRO TRABAJO NOSOTROS REQUERIMOS IDENTIFICAR QUIENES REALIZAN LAS COMPRAS PARA FUTURAS ESTRATEGIAS DE LEALTAD.
  - El punto de entrada es Program.cs (utilizando Top-level statements).
</context>

<task_description>
  Debes modificar 'Program.cs', si no existe debeá ser creado, para implementar un menú interactivo y profesional que permita al usuario ejecutar las acciones de gestión de productos. El sistema debe ser resiliente a errores de entrada y seguir el principio de Inversión de Dependencias.
</task_description>
